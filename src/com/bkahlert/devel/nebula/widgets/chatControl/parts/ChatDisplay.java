package com.bkahlert.devel.nebula.widgets.chatControl.parts;
...
import com.bkahlert.devel.nebula.widgets.chatControl.events.IChatDisplayListener;
import com.bkahlert.devel.nebula.widgets.chatControl.items.ChatLine;
import com.bkahlert.devel.nebula.widgets.chatControl.items.ChatLinePartnerChangeSeparator;
import com.bkahlert.devel.nebula.widgets.chatControl.items.ChatLineSeparator;
...
public class ChatDisplay extends ScrolledComposite {
	...
	public ChatDisplay(Composite parent, int style, Color backgroundColor) {
		...
		this.contentComposite.setBackground(backgroundColor);
	...
	protected void createOptionComposite() {
		...
		this.optionsComposite = new Composite(this.contentComposite, SWT.NONE);
		...
		final Button clearButton = new Button(this.optionsComposite, SWT.PUSH);
	...
	public void addChatLine(Object sender, Color color, String message,
		 ...
		 * Sender line
		 */
		if (this.lastUser != null && this.lastUser.equals(sender)) { // same
																		// user
			ChatLineSeparator chatLineSeparator = new ChatLineSeparator(
					this.contentComposite, sender.toString(), color, receivedOn);
		 ...
		 * Message line
		 */
		ChatLine chatLine = new ChatLine(this.contentComposite, message);
	...
	protected int computeMaxNonChatLineWidth() {
		...
		for (Control chatItem : this.getChatItems()) {
	...
	public void refresh() {
		...
		int widthHint = Math.max(this.computeMaxNonChatLineWidth()
