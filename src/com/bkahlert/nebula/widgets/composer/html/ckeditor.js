                    ...
                    if (d && d.type == CKEDITOR.NODE_TEXT)
                        if (f)
                            if (f >= d.getLength()) j.setStartAfter(d);
                                ...
                                j.setStartBefore(d);
                                k = 0
                            } else j.setStartBefore(d); if (g && g.type == CKEDITOR.NODE_TEXT)
                        if (e)
    ...
    (function () {
        function b(b, j, o) {
            ...
            if (j !== void 0) {
                ...
                } else throw Error("Expect element of type CKEDITOR.dom.element."); if (CKEDITOR.env.ie && CKEDITOR.env.quirks && o == CKEDITOR.ELEMENT_MODE_INLINE) throw Error("Inline element mode is not supported on IE quirks.");
    ...
    (function () {
        ...
        CKEDITOR.on("instanceCreated", function (a) {
            ...
            f.on("contentDom", function () {
                ...
                } else l.attachListener(l, "mouseup", c, f); if (CKEDITOR.env.webkit) a.on("keydown", function (a) {
        ...
        CKEDITOR.dom.selection.prototype = {
            ...
            unlock: function (a) {
                if (this.isLocked) {
                    ...
                    this.isLocked = 0;
                    this.reset();
                    if (a)(a = b || (c && c[0] && c[0].getCommonAncestor && c[0].getCommonAncestor())) && a.getAscendant("body", 1) && (b ? this.selectElement(b) : this.selectRanges(c))
                }
            },
            reset: function () {
                this._.cache = {}
...
*/
    ...
    (function () {
        var g, i = {
            ...
            exec: function (a) {
                ...
                if (b.fullPage) a = a.getData().replace(/<head>/, "$&" + e).replace(/[^>]*(?=<\/title>)/, "$& &mdash; " + a.lang.preview.preview);
                else {
                    var b = "<body ",
    ...
    CKEDITOR.plugins.add("onchange", {
        init: function (b) {
            function c() {
                b.readOnly || d || (d = setTimeout(function () {
                    d = 0;
                    b.fire("change")
                }, b.config.minimumChangeMilliseconds || 100))
            }
            var d, g = window.MutationObserver || window.WebKitMutationObserver || window.MozMutationObserver,
                f;
            b.on("destroy", function () {
                d && clearTimeout(d);
                d = null
            ...
            b.on("saveSnapshot", function (a) {
                (!a.data || !a.data.contentOnly) && c()
            });
            var e = b.getCommand("undo");
            e && e.on("afterUndo", c);
            (e = b.getCommand("redo")) && e.on("afterRedo", c);
            b.on("afterCommandExec",
                function (a) {
                    "source" != a.data.name && !1 !== a.data.command.canUndo && c()
                });
            g && (f = new g(function () {
                c()
            }), window.console && window.console.log && console.log("Detecting changes using MutationObservers"));
            b.on("contentDom", function () {
                f && setTimeout(function () {
                    f.observe(b.document.getBody().$, {
                        attributes: true,
                        childList: true,
                        characterData: true
                    })
                ...
                b.document.on("keydown", function (a) {
                    if (!a.data.$.ctrlKey && !a.data.$.metaKey) {
                        a = a.data.$.keyCode;
                        (a == 8 || a == 13 || a == 32 || a >= 46 && a <= 90 || a >= 96 && a <= 111 || a >= 186 && a <= 222 ||
                            a == 229) && c()
                    }
                });
                b.document.on("drop", c);
                b.document.getBody().on("drop", c)
            });
            b.on("mode", function () {
                if (b.mode == "source") {
                    var a = b.textarea || b._.editable;
                    a.on("keydown", function (a) {
                        !a.data.$.ctrlKey && !a.data.$.metaKey && c()
                    });
                    a.on("drop", c);
                    a.on("input", c);
                    if (CKEDITOR.env.ie) {
    ...
    CKEDITOR.config.plugins = 'dialogui,dialog,a11yhelp,dialogadvtab,basicstyles,blockquote,clipboard,button,panelbutton,panel,floatpanel,colorbutton,colordialog,templates,menu,contextmenu,div,resize,toolbar,elementspath,list,indent,enterkey,entities,popup,filebrowser,find,floatingspace,listblock,richcombo,font,fakeobjects,forms,format,htmlwriter,horizontalrule,image,justify,link,liststyle,magicline,maximize,newpage,pagebreak,pastetext,pastefromword,preview,print,removeformat,save,selectall,showblocks,showborders,sourcearea,specialchar,menubutton,scayt,stylescombo,tab,table,tabletools,undo,wsc,divarea,onchange';
    CKEDITOR.config.skin = 'moono';
    (function () {
        var icons = ('bold,0,italic,32,strike,64,subscript,96,superscript,128,underline,160,blockquote,192,copy-rtl,224,copy,256,cut-rtl,288,cut,320,paste-rtl,352,paste,384,bgcolor,416,textcolor,448,templates-rtl,480,templates,512,creatediv,544,bulletedlist-rtl,576,bulletedlist,608,numberedlist-rtl,640,numberedlist,672,indent-rtl,704,indent,736,outdent-rtl,768,outdent,800,find-rtl,832,find,864,replace,896,button,928,checkbox,960,form,992,hiddenfield,1024,imagebutton,1056,radio,1088,select-rtl,1120,select,1152,textarea-rtl,1184,textarea,1216,textfield-rtl,1248,textfield,1280,horizontalrule,1312,image,1344,justifyblock,1376,justifycenter,1408,justifyleft,1440,justifyright,1472,anchor-rtl,1504,anchor,1536,link,1568,unlink,1600,maximize,1632,newpage-rtl,1664,newpage,1696,pagebreak-rtl,1728,pagebreak,1760,pastetext-rtl,1792,pastetext,1824,pastefromword-rtl,1856,pastefromword,1888,preview-rtl,1920,preview,1952,print,1984,removeformat,2016,save,2048,selectall,2080,showblocks-rtl,2112,showblocks,2144,source-rtl,2176,source,2208,specialchar,2240,scayt,2272,table,2304,redo-rtl,2336,redo,2368,undo-rtl,2400,undo,2432,spellchecker,2464'),
