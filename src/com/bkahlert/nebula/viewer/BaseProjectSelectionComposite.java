...
public class BaseProjectSelectionComposite extends ProjectDisplayComposite {
	...
	protected ICheckStateListener checkStateListener = new ICheckStateListener() {
		...
		public void checkStateChanged(CheckStateChangedEvent event) {
			BaseProjectSelectionComposite.this.notifyProjectSelectionChanged(
					(IProject) event.getElement(), event.getChecked());
		}
	};
	...
	public BaseProjectSelectionComposite(Composite parent, int style) {
		...
		this.addDisposeListener(new DisposeListener() {
			...
			public void widgetDisposed(DisposeEvent e) {
				if (BaseProjectSelectionComposite.this.viewer != null) {
					((CheckboxTableViewer) BaseProjectSelectionComposite.this.viewer)
							.removeCheckStateListener(BaseProjectSelectionComposite.this.checkStateListener);
				}
			}
		});
	}
	...
	public void setSelectedProjects(List<IProject> projects) {
		...
		for (Map.Entry<IProject, Boolean> entry : checkStatesChanges.entrySet()) {
			this.notifyProjectSelectionChanged(entry.getKey(), entry.getValue());
		}
	}
